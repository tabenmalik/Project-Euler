...
class Problem:
    ...
    def run(self):
        euler_module = import_module(f'.euler_{self.n:03}', package='project_euler.euler_problems')
    ...
    def load_solutions(cls, force=False):
        if cls.solutions is None or force:
            solution_text = resources.files(project_euler).joinpath('solutions.txt').read_text()
